 ...
 * changes from a single database table and return them to the Synchronization
 * Server so that they can be synchronized to a destination endpoint.
 * This implementation does not use any configuration arguments.
 */
public final class ExampleJDBCSyncSource extends JDBCSyncSource
{
                                                       ...
                                                       throws SQLException
  {
    ...
    while(rset.next())
    {
      ...
      //the Sync Server will only modify the destination attributes that depend
      //on the originally changed source columns (i.e. if this is not set, no
      //modifications will take place to the destination entry).
